...
function PackagePreviewCard({ pkg }: { pkg: PublicPackage }) {
  ...
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-lg" style={{ background: '#fff', border: pkg.featured ? '1px solid #F2E9E4' : '1px solid rgba(9,15,59,0.12)', boxShadow: pkg.featured ? '0 14px 32px rgba(9,15,59,0.10)' : 'none' }}>
      <figure className="relative aspect-[16/10] overflow-hidden" style={{ background: '#090F3B' }}>
        ...
        <div className="absolute inset-x-[14px] top-[14px] flex justify-between gap-2">
          <span className="rounded-[6px] px-[8px] py-[4px] text-[10px] font-bold" style={{ background: '#090F3B', color: '#F2E9E4' }}>{pkg.tier}</span>
      ...
      <div className="flex flex-1 flex-col p-[16px]">
        <p className="flex items-center gap-[6px] text-[12px]" style={{ color: '#5D5D76' }}><CalendarDays size={14} color="#F2E9E4" />{pkg.dateLabel}</p>
        ...
        <div className="mt-[14px] space-y-[7px] text-[12px] leading-[1.4]" style={{ color: '#4B4F68' }}>
          <p className="flex gap-[8px]"><Clock3 size={15} className="mt-[1px] flex-none" color="#F2E9E4" />{pkg.duration}</p>
          <p className="flex gap-[8px]"><Plane size={15} className="mt-[1px] flex-none" color="#F2E9E4" />{pkg.airline}</p>
          <p className="flex gap-[8px]"><Hotel size={15} className="mt-[1px] flex-none" color="#F2E9E4" />{pkg.makkah}</p>
        </div>
      ...
      <div className="mx-auto max-w-[1180px]">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="flex flex-col justify-between gap-[24px] md:flex-row md:items-end">
          <div className="max-w-[700px]">
            <span className="inline-flex rounded-[6px] px-[9px] py-[5px] text-[11px] font-bold tracking-[0.12em] uppercase" style={{ background: '#090F3B', color: '#F2E9E4' }}>Paket Desember 2026</span>
