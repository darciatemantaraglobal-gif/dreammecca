...
function PackagePreviewCard({ pkg, whatsappNumber }: { pkg: PublicPackage; whatsappNumber?: string }) {
  ...
  return (
    <article className="package-preview-card flex h-full flex-col overflow-hidden rounded-lg" style={{ background: '#fff', border: pkg.featured ? '1px solid #C9ADA7' : '1px solid rgba(9,15,59,0.12)', boxShadow: pkg.featured ? '0 14px 32px rgba(9,15,59,0.10)' : 'none' }}>
      <figure className={`package-preview-media relative overflow-hidden ${pkg.poster ? 'aspect-[4/5]' : 'aspect-[16/10]'}`} style={{ background: '#090F3B' }}>
      ...
      <div className="package-preview-copy flex flex-1 flex-col p-[16px]">
        <p className="flex items-center gap-[6px] text-[12px]" style={{ color: '#5D5D76' }}><CalendarDays size={14} color="#C9ADA7" />{pkg.dateLabel}</p>
        <h3 className="package-preview-title mt-[8px] text-[20px] leading-[1.18]" style={{ color: '#090F3B', fontWeight: 700 }}>{pkg.title}</h3>
...
export default function Packages() {
  ...
  return (
    <section id="paket" className="mobile-compact-section px-[7vw] py-[72px] md:py-[132px]" style={{ background: '#F7F6F2' }}>
      <div className="mx-auto max-w-[1180px]">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true, amount: 0.3 }} variants={fadeUp} className="flex flex-col justify-between gap-[24px] md:flex-row md:items-end">
          <div className="max-w-[700px]">
            ...
            <h2 className="mobile-section-title mt-[14px] font-bold leading-[1.12]" style={{ fontSize: 'clamp(34px,4vw,54px)', color: '#090F3B', textWrap: 'balance' }}>Pilihan Program Umroh Terdekat</h2>
