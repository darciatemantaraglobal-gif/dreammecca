  ...
  return (
    <footer style={{ background: '#090F3B' }}>
      <div className="mobile-footer mx-auto grid max-w-[1180px] grid-cols-1 gap-[40px] px-[7vw] py-[56px] md:grid-cols-[1.15fr_0.85fr_0.85fr]">
        ...
        <div>
          ...
          <nav className="mt-[16px] flex flex-col items-start gap-[10px]">
            {content.navigation.map((link) => <a key={link.href} href={link.href} className="text-[14px] no-underline transition-opacity hover:opacity-100" style={{ color: 'rgba(255,255,255,0.63)' }}>{link.label}</a>)}
          </nav>
        </div>
        ...
        <div>
          ...
          <div className="mt-[16px] flex flex-col gap-[12px] text-[14px] leading-[1.55]" style={{ color: 'rgba(255,255,255,0.63)' }}>
            ...
            <a href={`mailto:${content.footer.email}`} className="flex items-center gap-[9px] no-underline" style={{ color: 'inherit' }}><Mail size={16} /> {content.footer.email}</a>
            <a href={instagramUrl} target="_blank" rel="noopener noreferrer" className="flex items-center gap-[9px] no-underline" style={{ color: 'inherit' }}><Instagram size={16} /> {content.footer.instagramLabel}</a>
      ...
      <div className="px-[7vw] py-[18px] text-[12px]" style={{ borderTop: '1px solid rgba(255,255,255,0.10)', color: 'rgba(255,255,255,0.45)' }}>
        © {content.footer.year} {content.footer.companyName}.
      </div>
    </footer>
  );
}
