  ...
  return (
    <section id="diskon" className="px-[7vw] py-[72px] md:py-[112px]" style={{ background: '#090F3B' }}>
      <div className="mx-auto grid max-w-[1180px] grid-cols-1 items-end gap-[32px] md:grid-cols-[0.8fr_1.2fr] md:gap-[72px]">
        <div>
          <span className="text-[12px] font-bold tracking-[0.14em] uppercase" style={{ color: '#F2E9E4' }}>Penawaran saat ini</span>
          ...
          <a href={createWALink(message)} target="_blank" rel="noopener noreferrer" className="mt-[24px] inline-flex min-h-11 items-center justify-center rounded-[6px] px-[20px] py-[12px] text-[14px] font-bold no-underline transition-opacity hover:opacity-90" style={{ background: '#F2E9E4', color: '#090F3B' }}>
            Konsultasi Promo
          </a>
        </div>
        ...
        <div className="grid grid-cols-1 gap-[10px] sm:grid-cols-2 md:gap-[14px]">
          {offers.map((offer) => (
            <article key={offer.title} className="rounded-lg p-[22px] md:p-[26px]" style={{ background: '#15205A', border: '1px solid rgba(226,188,108,0.28)' }}>
              <offer.icon size={22} color="#F2E9E4" strokeWidth={1.65} />
              <p className="mt-[28px] text-[13px] font-bold uppercase tracking-[0.12em]" style={{ color: '#F2E9E4' }}>{offer.title}</p>
